...
d[, id := stringr::str_pad(fips_county, width = 5, side = "right", pad = "0")]
...
d_viz <- merge(d, d_clean, by = c("year", "id"), all.x = TRUE)
...
perc <- setdiff(names(d_clean), c("year", "total_pop", "id"))
perc_name <- paste("perc", perc, sep = "_")

d_clean[, (perc_name) := lapply(.SD, \(x) {round(x * 100 / total_pop, 3)}), .SDcols = perc]
setkeyv(d_clean, c("year", "id"))
